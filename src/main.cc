 ...
 * @brief  Programa principal que leerá un archivo por comando junto a una
 * opción opcode que determinará la operación a realizar y mostrará en un
 * fichero de salida dichas operaciones.
 * @version 0.1
 * @date 2022-09-30
...
int main(int argc, char **argv) {
  ...
  if (verificado_comandos > 0)
    return verificado_comandos;
  ...
  std::vector<Cadena>
      vector_de_cadenas; // Contendrá cada cadena del archivo de lectura
  ...
  while (!archivo_entrada.eof()) {
    ...
    std::stringstream cadena_alfabeto(
        cadena_auxiliar_del_fichero); // Cada linea
    Alfabeto alfabeto;
    ...
    while (!cadena_alfabeto.eof()) {
      ...
      if (cadena_alfabeto.eof())
        break;
      alfabeto.Insert(auxiliar_alfabeto);
      iterador++;
    }
    if (cadena_auxiliar_del_fichero.empty())
      continue;
    cadena_alfabeto >> auxiliar_alfabeto;
    if (iterador == 0)
      alfabeto = Alfabeto(auxiliar_alfabeto);
  ...
  case 1:
    for (long unsigned int i = 0; i < vector_de_cadenas.size(); i++) {
      archivo_salida << vector_de_cadenas.at(i).length(
                            vector_de_alfabetos.at(i))
                     << std::endl;
    }
    break;
  case 2:
    for (long unsigned int i = 0; i < vector_de_cadenas.size(); i++) {
      archivo_salida << vector_de_cadenas.at(i).inversa(
                            vector_de_alfabetos.at(i))
                     << std::endl;
    }
    break;
  ...
  case 3:
    for (long unsigned int i = 0; i < vector_de_cadenas.size(); i++) {
      std::vector<Cadena> prefijos =
          vector_de_cadenas.at(i).Prefijos(vector_de_alfabetos.at(i));
  ...
  case 4:
    for (long unsigned int i = 0; i < vector_de_cadenas.size(); i++) {
      std::vector<Cadena> prefijos =
          vector_de_cadenas.at(i).Sufijos(vector_de_alfabetos.at(i));
      for (long unsigned int j = 0; j < prefijos.size(); j++) {
        archivo_salida << prefijos.at(j) << " ";
      }
      archivo_salida << "\n";
  ...
  case 5:
    for (long unsigned int i = 0; i < vector_de_cadenas.size(); i++) {
      std::set<Cadena> subcadenas =
          vector_de_cadenas.at(i).Subcadenas(vector_de_alfabetos.at(i));
