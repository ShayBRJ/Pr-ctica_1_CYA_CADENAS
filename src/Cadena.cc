...
Cadena::Cadena(std::string cadena_constructor) {
  this->secuencia = cadena_constructor;
}
...
int Cadena::length(Alfabeto simbolos) {
  ...
  for (int i = 0; i < this->secuencia.length(); i++) {
    auxiliar.clear();
    for (int j = 0;
         j <= simbolos.MaxSizeSymbol() && (j + i < secuencia.length()); j++) {
      auxiliar.push_back(secuencia.at(j + i));
      if (simbolos.getAlphabet().find(auxiliar) !=
              simbolos.getAlphabet().end() &&
          (auxiliar.size() > max)) {
        max = auxiliar.size();
        espacio = j;
      }
...
std::vector<Cadena> Cadena::Prefijos(Alfabeto simbolos) {
  ...
  for (int i = 0; i < this->secuencia.length(); i++) {
    for (int j = 0;
         j < simbolos.MaxSizeSymbol() && ((j + i) < this->secuencia.length());
         j++) {
    ...
    if (contador_vector != 1)
      prefijos_previos =
          auxiliar_cadenas.at(contador_vector - 1).getSecuencia();
...
std::vector<Cadena> Cadena::Sufijos(Alfabeto simbolos) {
  ...
  for (int i = this->secuencia.size() - 1; i >= 0; i--) {
    for (int t = 0; t < simbolos.MaxSizeSymbol() && ((i - t) >= 0); t++) {
      ...
      if (simbolos.getAlphabet().find(cadena_resultado) !=
              simbolos.getAlphabet().end() &&
          cadena_resultado.size() > max) {
    ...
    if (contador_vector != 1)
      sufijos_previos = auxiliar_cadenas.at(contador_vector - 1).getSecuencia();
